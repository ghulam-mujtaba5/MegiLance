...
export default function InvoicesPage() {
  ...
  const loadInvoices = async () => {
    try {
      ...
      // Mock data if API doesn't return invoices
      const mockInvoices: Invoice[] = response.invoices || [
        {
          id: 1,
      ...
      const pending = mockInvoices.filter(inv => ['sent', 'draft'].includes(inv.status)).reduce((sum, inv) => sum + inv.amount, 0);
