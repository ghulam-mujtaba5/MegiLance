  ...
  const fetchWorkflows = async () => {
    ...
    try {
      ...
      const mockWorkflows: Workflow[] = [
        {
          id: 'wf_001',
          name: 'Auto-respond to proposals',
          description: 'Send acknowledgment when new proposal received',
          trigger: { type: 'proposal_received', config: {} },
          actions: [{ type: 'send_message', config: { template: 'proposal_ack' } }],
          isActive: true,
        ...
        },
        {
          ...
          description: 'Notify client when milestone is marked complete',
          trigger: { type: 'milestone_completed', config: {} },
          actions: [
            { type: 'send_email', config: { to: 'client', template: 'milestone_done' } },
            { type: 'create_invoice', config: { auto: true } }
          ],
          isActive: true,
        ...
        },
        {
          ...
          description: 'Send weekly summary of tracked hours',
          trigger: { type: 'schedule', config: { cron: '0 9 * * 1' } },
          actions: [{ type: 'send_report', config: { type: 'time_summary' } }],
          isActive: false,
          runCount: 3,
            ...
            <div className={commonStyles.workflowsList}>
              {workflows.length === 0 ? (
                ...
                workflows.map(workflow => (
                  <div key={workflow.id} className={cn(commonStyles.workflowCard, themeStyles.workflowCard)}>
          ...
          {activeTab === 'templates' && (
            <div className={commonStyles.templatesGrid}>
              {templates.map(template => (
                <div key={template.id} className={cn(commonStyles.templateCard, themeStyles.templateCard)}>
            ...
            <div className={cn(commonStyles.logsPanel, themeStyles.logsPanel)}>
              <div className={commonStyles.logsList}>
                {[
                  ...
                  <div key={log.id} className={cn(commonStyles.logItem, themeStyles.logItem)}>
                    <div className={cn(
