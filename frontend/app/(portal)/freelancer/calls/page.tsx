...
export default function VideoCallsPage() {
  ...
  const loadCalls = async () => {
    try {
      setLoading(true);
      const response = await videoCallsApi.getHistory().catch(() => []);
      
      // Mock data
      const mockCalls: VideoCall[] = response.calls || [
        {
          id: '1',
          room_id: 'MEET-ABC123',
      ...
      <div className={commonStyles.callsList}>
        {displayCalls.length === 0 ? (
          ...
          displayCalls.map((call) => (
            <div key={call.id} className={cn(commonStyles.callCard, themeStyles.callCard)}>
