...
async def match_freelancers_to_project(
    ...
    try:
        ...
        freelancers_result = execute_query(
            """SELECT u.id, u.name, u.email, u.skills, u.hourly_rate, COALESCE(u.hourly_rate, 50),
                      u.profile_image_url, u.bio, 0 as completed_projects
               FROM users u
...
async def estimate_freelancer_hourly_rate(
    ...
    try:
        # Get freelancer data
        result = execute_query(
            """SELECT id, name, skills, hourly_rate, rating, completed_projects,
                      years_experience
               FROM users
...
async def get_profile_optimization_suggestions(
    ...
    try:
        result = execute_query(
            """SELECT id, name, bio, skills, hourly_rate, portfolio_url,
